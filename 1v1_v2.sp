...
#define TAG "\x01[\x0Eroby 1v1\x01]"
public Plugin myinfo = 
{
	name = "1v1 plugin",
	author = "roby/awyx", // Thanks D34Dspy for helping
	description = "",
	version = "2",
...
public void Info1v1()
{
	if (g_bIsMatchLive) 
	{
		PrintToChatAll("%s", TAG);
		PrintToChatAll("\x01 \x04************************************************************");
		PrintToChatAll("\x01 \x03Match: \x0C%s \x01vs \x0F%s", g_sPlayer1Name, g_sPlayer2Name); 
		PrintToChatAll("\x01 \x03Score: \x0C%d \x01- \x0F%d \x01(First to \x05%d\x01)", g_iRoundsWon_p1, g_iRoundsWon_p2, g_iNRounds); 
...
public Action Event_PlayerDisconnect(Event event, const char[] name, bool dontBroadcast)
{
	if (g_bIsMatchLive)
	{
		...
		if (client == g_iPlayer1 || client == g_iPlayer2){
			g_bPlayerLeft = true;
			PrintToChatAll("%s \x0FPlayer have disconnected!", TAG);
			Cancel1v1(0,0);
		}
	}
}  
